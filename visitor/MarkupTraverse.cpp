														 ...
														 double a_dParallelToScreenPMIScale)
{
	...
	A3DTessMarkupData sMarkupTessData = A3D_MAKE_DATA(A3DTessMarkupData);
	CHECK_RET(A3DTessMarkupGet(m_sMarkupData.m_pTessellation, &sMarkupTessData));

	A3DTessBaseData sTessBaseData = A3D_MAKE_DATA(A3DTessBaseData);
	...
	for(uI = 0; uI < m_sMarkupData.m_uiLeadersSize; uI++)
	{
		...
		A3DMkpLeaderData sMarkupLeaderData = A3D_MAKE_DATA(A3DMkpLeaderData);
		CHECK_RET(A3DMkpLeaderGet(psLeader, &sMarkupLeaderData));

		A3DTessMarkupData sMarkupLeaderTessData = A3D_MAKE_DATA(A3DTessMarkupData);
		CHECK_RET(A3DTessMarkupGet(sMarkupLeaderData.m_pTessellation, &sMarkupLeaderTessData));

		A3DTessBaseData sLeaderTessBaseData = A3D_MAKE_DATA(A3DTessBaseData);
