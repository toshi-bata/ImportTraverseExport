													...
													bool bVisitPrototype)
{
	...
	if (psVisitor->GetFlagElementToConnect() & CONNECT_PMI)
	{
		...
		if (!uNbAnnotationEntity && m_sProductOccurrenceData.m_pPrototype != A3D_NULL_HANDLE)
		{
			...
			{
				A3DAsmProductOccurrenceData sProductPrototypeData = A3D_MAKE_DATA(A3DAsmProductOccurrenceData);
...
A3DStatus A3DRiBrepModelConnector::TraverseRiBrepModel(A3DVisitorContainer*  psVisitor)	  const
{
	...
	{
		A3DRiRepresentationItemData sRidata = A3D_MAKE_DATA(A3DRiRepresentationItemData);
