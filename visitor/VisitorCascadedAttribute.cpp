...
A3DStatus A3DVisitorColorMaterials::visitEnter(const A3DProductOccurrenceConnector& sEntity)
{
	...
	/*	A3DEEntityType eType;
		...
		{
			A3DMiscReferenceOnTopologyData sTopoRefData = A3D_MAKE_DATA(A3DMiscReferenceOnTopologyData);
			A3DMiscReferenceOnTopologyGet(sEntity.m_sProductOccurrenceData.m_ppEntityReferences[uI], &sTopoRefData);
			...
			A3DMiscReferenceOnTopologyGet(NUL, &sTopoRefData));

			A3DTopoBrepData* pBrepData = sTopoRefData.m_pBrepData;
			A3DTopoBrepDataData sTopoBrepDataData = A3D_MAKE_DATA(A3DTopoBrepDataData);
			...
			if(iErr == A3D_SUCCESS)
			{
				...
				if(eType == kA3DTypeMiscEntityReference || eType==kA3DTypeMiscMarkupLinkedItem)
				{
					...
					else // no topo -> global PO/Part/RI/RIBrepModel
					{
						...
						A3DMiscCascadedAttributesData sCAData = A3D_MAKE_DATA(A3DMiscCascadedAttributesData);
