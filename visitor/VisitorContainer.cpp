...
A3DStatus A3DVisitorContainer::visitEnter(const A3DProductOccurrenceConnector& sConnector)
{
	...
	{
		A3DMiscEntityReferenceData sMiscRefData = A3D_MAKE_DATA(A3DMiscEntityReferenceData);
		...
		{
			A3DMiscReferenceOnCsysItemData sA3DMiscReferenceOnCSYSITemData = A3D_MAKE_DATA(A3DMiscReferenceOnCsysItemData);
...
A3DStatus A3DVisitorContainer::visitEnter(const A3DMkpViewConnector& sConnector)
{
	...
	for (uI = 0; uI < uNbRefEntity; uI++)
	{
		A3DMiscMarkupLinkedItemData sMkpLinkedItemData = A3D_MAKE_DATA(A3DMiscMarkupLinkedItemData);
